...
function build_kdtree(points, depth = 0) {
    ...
    var mediana = Math.floor(points.length / 2);

    var node = new Node(points[mediana], axis);

    node.left = build_kdtree(points.slice(0, mediana), depth + 1);
    node.right = build_kdtree(points.slice(mediana + 1, points.length), depth + 1);
